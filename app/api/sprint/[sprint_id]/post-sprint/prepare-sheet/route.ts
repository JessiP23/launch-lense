...
export async function POST(
  ...
  try {
    ...
    if (!verdict) {
      return Response.json({ error: 'SpreadsheetAgent waits for an aggregate verdict before preparing outreach.' }, { status: 409 });
    }
    if (verdict !== 'GO' && verdict !== 'ITERATE') {
      return Response.json(
        { error: 'SpreadsheetAgent activates only when aggregate verdict is GO or ITERATE. NO-GO blocks outreach.' },
        { status: verdict === 'NO-GO' ? 403 : 409 },
      );
    }
    ...
    const message = e instanceof Error ? e.message : 'SpreadsheetAgent failed';
    console.error('[prepare-sheet] failed:', e);
